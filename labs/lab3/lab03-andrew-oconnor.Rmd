...
```{r}
a <- filter(starwars, homeworld == "Alderaan")
select(a, name)
```
...
```{r}
filter(starwars, skin_color == "green")
```
...
```{r}
b <- filter(starwars, sex == "male" & hair_color == "black" | sex == "male" & hair_color == "brown")
select(b, name, height, mass)
```
...
```{r}
c <- filter(starwars, species == "human" & sex == "female")
...
```{r}
e <- select(arrange(starwars, desc(height)), name, homeworld, species)
slice(e, 1:5)
```
...
```{r}
f <- arrange(starwars, desc(mass))
g <- select(f, name, homeworld, species)
slice(g, 1:5)
```
...
```{r}
ggplot(data = fem, aes(x=height, y=mass)) + geom_point(aes(color = name))
```
...
```{r}
all_fem = filter(starwars, sex == "female")
ggplot(data = all_fem, aes(x = height, y = mass)) + geom_point() + geom_label(aes(label = name))
```
...
```{r}
ggplot(starwars, aes(x = mass)) + geom_histogram()
```
