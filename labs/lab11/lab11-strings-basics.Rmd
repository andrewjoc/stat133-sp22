...
```{r}
paste(states, num_chars, sep =" = ")[1:5]
```
...
```{r}
paste(substr(states, 1, 1), substr(states, num_chars - 2, stop= num_chars), sep = "")
```
...
```{r}
states_list <- list()
for (i in as.numeric(names(char_freqs))) {
  if (i %% 2 == 0) {
    states_list [[paste(i, 'chars', sep = "-")]] <- toupper(paste(states[num_chars == i], collapse = ", ")) }
  else {
    states_list [[paste(i, 'chars', sep = "-")]] <- tolower(paste(states[num_chars == i], collapse = ", "))
  }
}
