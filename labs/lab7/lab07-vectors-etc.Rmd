...
__factor__. Internally, an R `factor` is stored as an integer vector, and 
...
```{r}
female_matrix <- matrix(c(mass[sex == "female"], height[sex == "female"]), ncol = 2)
female_matrix
```
...
```{r}
colnames(female_matrix) <- c("mass", "height")
rownames(female_matrix) <- name[sex == "female"]
female_matrix
```
