...
length(tweets$author[str_length(tweets$author) > 15])
```
...
```{r}
tweets$author[str_which(tweets$author, "\\W")]
```
...
```{r}
min(str_length(tweets$author))
tweets$author[str_length(tweets$author) == 2]
```
...
d) Display the first 10 elements of tweets that do NOT contain the characters `"a"` or `"A"` _(write a command to answer this question)_.

```{r}
twts <- tweets$content[!str_detect(tweets$content, "[aA]")]
...
```{r}
summary(str_count(tweets$content, "!"))
```
...
```{r}
tweets$content[str_count(tweets$content, '!') == max(str_count(tweets$content, '!'))]
```
...
```{r}
tab <- table(str_count(tweets$content, '#[[:alpha:]][[:alnum:]]+'))
```
...
```{r}
summary(str_count(unique(unlist(str_extract_all(tweets$content, '#[[:alpha:]][[:alnum:]]+'))))) - 1
```
...
```{r}
table(str_count(unique(unlist(str_extract_all(tweets$content, '#[[:alpha:]][[:alnum:]]+')))) - 1)
```
...
tweets %>%
  group_by(sentiment) %>%
  summarise(avg_low = mean(str_count(content, '[[:lower:]]')), 
            avg_up = mean(str_count(content, '[[:upper:]]')),
            dig = mean(str_count(content, '[[:digit:]]')),
            punct = mean(str_count(content, '[[:punct:]]')),
            spaces = mean(str_count(content, '[[:space:]]'))
            )
```
